...
interface SwishModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (fromUser: string, toUser: string, amount: number) => Promise<void>;
  members: GroupMember[];
...
export function SwishModal({
  ...
  const handleSubmit = async () => {
    ...
    try {
      await onSubmit(currentUserId, toUserId, numericAmount);
            ...
            <div className="bg-background border border-border rounded-md p-6 w-full max-w-sm">
              ...
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-2">
                  <div className="p-1.5 rounded-md bg-[#0CA677]/10">
                    <Smartphone size={18} className="text-[#0CA677]" />
                  </div>
              ...
              <p className="text-muted-foreground text-sm mb-6">
                Registrera att du har Swishat pengar till en annan medlem.
              </p>
              ...
              <div className="space-y-4">
                ...
                {/* Preview */}
                {toUserId && amount && parseFloat(amount) > 0 && (
                  <motion.div
                  ...
                  >
                    <p className="text-sm text-muted-foreground mb-1">
                      {currentMember?.name} → {selectedMember?.name}
                    </p>
